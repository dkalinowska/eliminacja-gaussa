	...
	if (fin != NULL) 
	{
		...
		{			
			for (ir = 0; ir < r; ir++) 			
	       	...
	       	else 	fprintf(stderr,"Wystąpił problem podczas tworzenia macierzy o rozmiarach %d x %d dla danych z pliku: %s\n", r, c, fname);
		fclose(fin);
	} 
	
	else 	fprintf(stderr,"Nie mogę otworzyć pliku o nazwie: %s\n", fname);
